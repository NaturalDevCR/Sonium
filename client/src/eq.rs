...
struct BiquadState {
    ...
    a1: f32, a2: f32,   // already divided by a0
...
impl BandFilter {
    fn new(band: &EqBand, sample_rate: u32, n_channels: usize) -> Self {
        ...
        Self { channels: vec![proto; n_channels] }
    }
...
impl EqProcessor {
    ...
    pub fn new(bands: &[EqBand], sample_rate: u32, n_channels: usize) -> Self {
        Self {
            bands:      bands.iter().map(|b| BandFilter::new(b, sample_rate, n_channels)).collect(),
            n_channels,
        }
    }
