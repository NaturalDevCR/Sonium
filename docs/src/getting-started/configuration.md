...
compensate so all speakers stay in sync. As in Snapcast, the value is
subtracted from the buffer: a positive `latency_ms` makes that speaker play
*earlier* (playout delay = `buffer_ms − latency_ms`, never below 0), so keep it
below `buffer_ms`. Negative values delay a speaker. Releases before 0.1.93
added the value instead, which pushed compensated speakers further behind.
