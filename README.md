...
**Recently Completed (v0.1.93):**
- Real announcement/TTS/URL playback with temporary volume and automatic
  resume; Home Assistant media browser, zone volume, zeroconf, auto re-login.
- Snapcast-style real-time pacing of stream sources (files, MPD/Mopidy FIFOs,
  ffmpeg without `-re`, HTTP bursts) with continuous timestamps.
- Sync fixes: per-client latency now compensates in the right direction
  (positive = play earlier) and clients keep their own clock offset instead of
  being pulled to a group median; perceptual, click-free software volume.
