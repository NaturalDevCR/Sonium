...
fn to_channels(n: u16) -> Result<Channels> {
    match n {
        ...
        c => Err(SoniumError::Codec(format!("unsupported Opus channel count {c}"))),
    }
}
...
fn to_sample_rate(r: u32) -> Result<SampleRate> {
    match r {
        ...
        r     => Err(SoniumError::Codec(format!("unsupported Opus sample rate {r}"))),
    }
}
...
impl OpusDecoder {
    pub fn from_header(header_data: &[u8]) -> Result<Self> {
        ...
        Ok(Self { inner, fmt: SampleFormat::new(rate, bits, channels) })
    }
}
...
impl OpusEncoder {
    pub fn new(fmt: SampleFormat) -> Result<Self> {
        let inner = AudEncoder::new(to_sample_rate(fmt.rate)?, to_channels(fmt.channels)?, Application::Audio)
...
impl Encoder for OpusEncoder {
    ...
    fn sample_format(&self) -> SampleFormat { self.fmt }
    fn codec_name(&self) -> &'static str { "opus" }
