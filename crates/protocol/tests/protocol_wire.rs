...
use sonium_protocol::{
    ...
    messages::{
        ...
        ClientInfo, CodecHeader, ErrorMsg, Hello, Message, ServerSettings, TimeMsg, WireChunk,
    },
...
fn encode_time_request_matches_fixture() {
    ...
    assert_eq!(
        &encoded[0..4],
        &TIME_REQUEST_FIXTURE[0..4],
        "type+id mismatch"
    );
    assert_eq!(
        &encoded[HEADER_SIZE..],
        &TIME_REQUEST_FIXTURE[HEADER_SIZE..],
        "payload mismatch"
    );
}
...
fn simulate_client_connection_sequence() {
    ...
    let ci = wire(&Message::ClientInfo(ClientInfo {
        volume: 75,
        muted: false,
    }));
